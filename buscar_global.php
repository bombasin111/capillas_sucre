...
if (!isset($_SESSION['busqueda_global'])) {
    echo "Error: No estás autenticado para la búsqueda global.<br>"; // Depuración
...
if (isset($_GET['buscar'])) {
    ...
    $busqueda = "%$busqueda%"; // Agregamos los % para la búsqueda parcial

    // Consulta SQL para buscar en todas las iglesias
    $query = $conexion->prepare("
        SELECT f.nombre, f.ci, f.fecha_nacimiento, f.bautizo, f.confirmacion, f.matrimonio, i.nombre AS iglesia
        FROM feligresespt f
        JOIN iglesiaspt i ON f.id_iglesia = i.id
        WHERE f.nombre ILIKE ?
    ");
    $query->execute([$busqueda]); // Pasamos el valor dos veces (para nombre y CI)
